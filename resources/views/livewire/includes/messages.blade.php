...
<div class="flex flex-col justify-center">
    <div class="fixed right-2 bottom-2 bg-{{ session('message-type') == 'danger' ? 'red-600' : 'green-500' }} shadow-lg mx-auto w-96 max-w-full text-sm pointer-events-auto bg-clip-padding rounded-lg block mb-3" id="static-example" role="alert" aria-live="assertive" style="z-index: 9999999">
        <div class="bg-{{ session('message-type') == 'danger' ? 'red-600' : 'green-500' }} flex justify-between items-center py-2 px-3 bg-clip-padding border-b border-{{ session('message-type') == 'danger' ? 'red-600' : 'green-500' }} rounded-t-lg rounded-b-lg">
            <p class="text-black justify-center items-center font-normal">
                {!! session('message') !!}
            </p>
